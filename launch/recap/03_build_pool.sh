...
# 用法: 03_build_pool.sh <task> <tag> <expert_dataset_dir> [expert_episodes=all]
...
ds = LeRobotDataset("local/expert", root=src)
split_dataset(ds, splits={"sub": list(range(n))}, output_dir=out + "_splits")
...
import json; sc=json.load(open('$EXP_V30/episode_success.json')); sc['episodes']=[e for e in sc['episodes'] if e['episode_index']<$NSUB]; sc['saved_episode_count']=len(sc['episodes']); json.dump(sc,open('$WORK/expert_v30/episode_success.json','w'),indent=2)"; fi
    NEXP=$NSUB

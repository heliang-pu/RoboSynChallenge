...
# 监控: tail -f outputs/value_train/value_<task>_<tag>.launch/train.log ;wandb 链接见日志 "Track this run"
...
# 启动脚本与日志放在旁边的 .launch 目录:训练器要求 output_dir 不存在(否则 FileExistsError)
LAUNCH=${OUT}.launch; mkdir -p "$LAUNCH"; need_file "$MODEL_SIGLIP"; need_file "$MODEL_LM"
...
EOS
chmod +x "$LAUNCH/train_cmd.sh"
pid=$(detach "$LAUNCH/train.log" bash "$LAUNCH/train_cmd.sh")
log "已脱离会话启动 pid=$pid  日志: $LAUNCH/train.log"
for i in $(seq 1 30); do sleep 5; grep -aqE "Track this run|Traceback|Error" "$LAUNCH/train.log" 2>/dev/null && break; done
grep -aqE "Traceback|Error" "$LAUNCH/train.log" 2>/dev/null && die "启动失败,见 $LAUNCH/train.log"
grep -a "Track this run" "$LAUNCH/train.log" 2>/dev/null || log "警告: 150s 内未见 wandb 链接,看 $LAUNCH/train.log"
log "约 $(awk -v s="$STEPS" 'BEGIN{printf "%.1f", s*5.4/3600}') 小时训完;建议 ~3000 步时跑: 05_value_qc.sh $TASK $TAG 001500 002000 003000(只列已存在的档)"

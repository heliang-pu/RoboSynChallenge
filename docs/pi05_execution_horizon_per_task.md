...
第五轮（H=30 复验与补全，100 集 / Hybrid）：

| 任务 | H | seed | 成功率 | AE | Score | 机器 |
|---|---:|---:|---:|---:|---:|---|
| items_handover | 30 | 1（换种子复验） | **68%** | 6.6 | 52.3 | 4090（seed 0 为 61%） |
| item_assembly | 30 | 1（换种子复验） | **59%** | 16.2 | 47.5 | 4090（seed 0 为 59%，两批种子完全不重叠） |
| water_pouring | 30 | 0 | 61% | 36.3 | 53.0 | PRO 6000（10：76%，50：52%，单调） |
| mixer_operating | 30 | 0 | 79% | 35.5 | 66.3 | PRO 6000（10：78–79%，50：80%） |

换种子后 handover/item_assembly 的 H=30 优势都站住了（handover 61–68% vs H=50 35%；item_assembly 59% vs 45%），
可以从「一次 100 集」升为**确定**。water_pouring 三档单调（76/61/52），取 10；mixer 三档成功率持平，按 AE 取 50。

### ft67500 系列最佳存档的官方条件复测（Hybrid，100 集 / seed 0）

表 B 里各任务选出的存档（20 集选出，有选择偏差），在 RTX 机器上按 Hybrid 复测 100 集，与 28000 权重的最佳 H 对比：

| 任务 | ft67500 存档 | H | 成功率 | AE | Score | 28000 最佳（H） | 谁更好 |
|---|---|---:|---:|---:|---:|---|---|
| drawer_open_place | ck16999 | 50 | **56%** | 31.2 | **48.2** | 20%（50）· 17.2 | **ft67500 +31**（ACT v21 同条件 52%） |
| water_pouring | ck8080 | 50 | **85%** | 52.3 | **74.2** | 76%（10）· 63.2 | ft67500 +11 |
| table_rearrangement | ck7999 | 10 | **77%** | 40.8 | **65.9** | 70%（10）· 60.1 | ft67500 +5.8 |
| handle_basket | ck3336 | 10 | 86% | 13.2 | 67.1 | 90%（10）· 70.5 | 28000 |
| click_bell | ck2220 | 10 | 67% | 54.9 | 61.2 | 76%（10）· 69.7 | 28000 |
| mixer_operating | ck12639 | 50 | 67% | 32.2 | 56.7 | 80%（50）· 67.5 | 28000 |
| manipulate_pipette | ck8580 | 10 | 62% | 43.4 | 55.2 | 71%（10）· 63.3 | 28000 |
| items_handover | ck3270 | 10 | 17% | 0.6 | 12.9 | 61%（30）· 47.0 | 28000（ft67500 只测了 H=10） |

按任务混选两批权重（drawer / water / table 用 ft67500，其余用 28000）：**宏平均 SR 65.7%，Overall 55.5**
（纯 28000 取优 50.8）。drawer 是最大的单项收益：ft67500 ck16999 在 Hybrid 下 56%，比 28000 权重高 36 个点，
和本机 ACT v21 的 52% 处于同一水平。

### 复测后的建议配置（待拍板）

| 任务 | 定稿值 | 复测建议 | 单项分变化 | 依据 |
|---|---:|---:|---:|---|
| water_pouring | 50 | **10** | +17.9 | 10/30/50 = 76/61/52%，两机 H=50 都 52% |
| items_handover | 50 | **30** | +19.9 | 61%（seed 0）/ 68%（seed 1）vs 35%，已换种子复验 |
| item_assembly | 50 | **30** | +11.3 | 59%（seed 0）/ 59%（seed 1）vs 45%，已换种子复验 |

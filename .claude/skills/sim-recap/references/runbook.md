...
| 1 rollout | `bash launch/recap/01_rollout.sh <task> <tag> pi05_<task> <task> <ckpt_id> <episodes> [shards=2]`(同一策略再采一批加 `SEED_BASE=30011`,否则场景重复) | 分片并行(各自 seed)→ 合并 → `lerobot_dataset/rollouts/<task>_<tag>`(v2.1+边车)+ 工作目录 `rollout_v30/rollout_merged`。GPU 独占 ~1 分钟/集 |
...
| 2-4 数据池 | `bash launch/recap/03_build_pool.sh <task> <tag> <专家目录> [expert_episodes]`(`all` / 前 N 集 / `A:B` 区间,如 `200:260` 取训练池之外的专家集做 held-out) | 专家 v2.1→v3.0(指纹缓存,边车随带)→ 可选子集 → 合并 `[专家, rollout]` → 写 `episode_success`。round1 用 200 专家:150 rollout |

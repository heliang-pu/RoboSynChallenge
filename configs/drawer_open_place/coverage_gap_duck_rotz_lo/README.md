# drawer_open_place / coverage_gap_duck_rotz_lo

- 用途: 密度缺口补采(kind=marginal)
- 建议集数: 135
- 事件模式: keep-original(仅收窄官方事件范围)
- 依据: duck_rotz in [-30.000, -7.500] holds 0 kept episodes vs 374.2 expected (8-bin uniform)
...
| duck | rotation_range | [[0, 0, -30], [0, 0, 30]] | [[0.0, 0.0, -30.0], [0.0, 0.0, -7.5]] |

## 采集

```bash
python -m scripts.run_env \
    --gym_config configs/drawer_open_place/coverage_gap_duck_rotz_lo/gym_config.json \
    --action_config configs/drawer_open_place/action_config.json \
    --num_envs 1 --max_episodes 135 --headless --report_task_success
```

输出数据集: `lerobot_dataset/coverage/drawer_open_place/coverage_gap_duck_rotz_lo`

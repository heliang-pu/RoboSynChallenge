# item_assembly / coverage_strat_guijiao111

- 用途: 全范围分层补匀(kind=strat)
- 建议集数: 37
- 事件模式: pair-constrained(约束对采样)
- 依据: stratified xy quadrant of guijiao1 for uniform replenishment over the full official range: guijiao1_dx[0.000,0.010] guijiao1_dy[0.000,0.010]
...
| 对象 | 参数 | 官方 random | 本配置 |
|---|---|---|---|
| guijiao1 | position_range | [[-0.01, -0.01, 0.0], [0.01, 0.01, 0.0]] | [[0.0, 0.0, 0.0], [0.01, 0.01, 0.0]] |
...
python -m scripts.run_env \
    --gym_config configs/item_assembly/coverage_strat_guijiao111/gym_config.json \
    --action_config configs/item_assembly/action_config.json \
    --num_envs 1 --max_episodes 37 --headless --report_task_success
```

输出数据集: `lerobot_dataset/coverage/item_assembly/coverage_strat_guijiao111`

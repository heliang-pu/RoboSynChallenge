# item_assembly / coverage_gap_guijiao2_xy_xlo_ylo

- 用途: 密度缺口补采(kind=xy)
- 建议集数: 55
- 事件模式: pair-constrained(约束对采样)
- 依据: guijiao2 xy block x[-0.010,-0.003] y[-0.010,-0.003] holds 17 kept episodes vs 63.8 expected (4 grid cells)
...
| guijiao2 | position_range | [[-0.01, -0.01, 0.0], [0.01, 0.01, 0.0]] | [[-0.01, -0.01, 0.0], [-0.00333, -0.00333, 0.0]] |
...
python -m scripts.run_env \
    --gym_config configs/item_assembly/coverage_gap_guijiao2_xy_xlo_ylo/gym_config.json \
    --action_config configs/item_assembly/action_config.json \
    --num_envs 1 --max_episodes 55 --headless --report_task_success
```

输出数据集: `lerobot_dataset/coverage/item_assembly/coverage_gap_guijiao2_xy_xlo_ylo`

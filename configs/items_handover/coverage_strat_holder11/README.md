# items_handover / coverage_strat_holder11

- 用途: 全范围分层补匀(kind=strat)
- 建议集数: 37
- 事件模式: pair-constrained(约束对采样)
- 依据: stratified xy quadrant of holder for uniform replenishment over the full official range: holder_x[0.600,0.700] holder_y[0.125,0.250]
...
| holder | position_range | [[0.5, 0.0, 0.884], [0.7, 0.25, 0.884]] | [[0.6, 0.125, 0.884], [0.7, 0.25, 0.884]] |
...
python -m scripts.run_env \
    --gym_config configs/items_handover/coverage_strat_holder11/gym_config.json \
    --action_config configs/items_handover/action_config.json \
    --num_envs 1 --max_episodes 37 --headless --report_task_success
```

输出数据集: `lerobot_dataset/coverage/items_handover/coverage_strat_holder11`

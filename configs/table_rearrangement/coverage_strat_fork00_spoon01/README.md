# table_rearrangement / coverage_strat_fork00_spoon01

- 用途: 全范围分层补匀(kind=strat)
- 建议集数: 32
- 事件模式: pair-constrained(约束对采样)
- 依据: stratified block over the official random range (no pose columns in the dataset): fork_x[0.400,0.525] fork_y[0.150,0.225] spoon_x[0.400,0.525] spoon_y[-0.225,-0.150]
...
python -m scripts.run_env \
    --gym_config configs/table_rearrangement/coverage_strat_fork00_spoon01/gym_config.json \
    --action_config configs/table_rearrangement/action_config.json \
    --num_envs 1 --max_episodes 32 --headless --report_task_success
```

输出数据集: `lerobot_dataset/coverage/table_rearrangement/coverage_strat_fork00_spoon01`

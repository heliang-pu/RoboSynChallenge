# handle_basket / coverage_strat_milk10_basket00

- 用途: 全范围分层补匀(kind=strat)
- 建议集数: 31
- 事件模式: keep-original(仅收窄官方事件范围)
- 依据: stratified block over the official random range (no pose columns in the dataset): basket_x[0.600,0.650] basket_y[-0.150,-0.125] milk_x[0.675,0.750] milk_y[0.050,0.175]
...
python -m scripts.run_env_seeded \
    --gym_config configs/handle_basket/coverage_strat_milk10_basket00/gym_config.json \
    --action_config configs/handle_basket/action_config.json \
    --num_envs 1 --max_episodes 31 --headless --report_task_success
```

输出数据集: `lerobot_dataset/coverage/handle_basket/coverage_strat_milk10_basket00`

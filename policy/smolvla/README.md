...
This folder adds SmolVLA finetuning and evaluation support to
RoboSynChallenge.

## Contents

- [Environment Setup](#environment-setup)
- [Generate RoboSynChallenge Data](#generate-robosynchallenge-data)
- [Prepare SmolVLA Data for Training](#prepare-smolvla-data-for-training)
- [Write the Corresponding Training Configuration](#write-the-corresponding-training-configuration)
- [Finetune Model](#finetune-model)
- [Eval on RoboSynChallenge](#eval-on-robosynchallenge)
- [Troubleshooting](#troubleshooting)

## Environment Setup

Install RoboSynChallenge and EmbodiChain following the main repository
installation guide. For local installation, the expected workspace layout is:
...
SmolVLA also needs LeRobot. The adapter supports two common setups.

There are two Python contexts during evaluation:
...
They may be the same Python environment, but they do not have to be. If you use
separate environments, set `SMOLVLA_PYTHON` for the worker and keep the
RoboSynChallenge/EmbodiChain environment active for launching `eval.sh`.

### Use an Existing SmolVLA Environment

If you already have a Python environment that can import `lerobot` and run
...
If you only have the RoboSynChallenge repository, clone and install LeRobot
...
If this same active Python environment should also run the SmolVLA worker, set:
...
See the RoboSynChallenge data collection documentation for details. This SmolVLA
wrapper assumes the task dataset is already exported in a valid LeRobot format.

## Prepare SmolVLA Data for Training

SmolVLA training reads the RoboSynChallenge LeRobot dataset directly from
`--dataset.root`. You do not need to copy the dataset into `policy/smolvla`.
...
Extra arguments passed after `gpu_id` are appended to `lerobot-train`, so they

#!/usr/bin/env bash
# =============================================================================
# pi0.5 单任务微调脚本 —— water_pouring
# =============================================================================
#
# 用途:
#   在 RoboSynChallenge 的 water_pouring 任务数据集上全量微调官方 pi05_base 模型。
#
# 训练配置(定义在 src/openpi/training/config.py 的 "pi05_water_pouring"):
#   - 基座模型   : gs://openpi-assets/checkpoints/pi05_base/params(首次运行自动下载)
#   - 模型结构   : Pi0Config(pi05=True, action_horizon=50) —— 每次预测 50 步动作块
#   - 数据集     : RoboSynChallenge/cobotmagic_Sim_water_pouring
...
#
# 用法:
#   ./train_water_pouring.sh [GPU_ID] [EXP_NAME] [extra args...]
#
#   GPU_ID    : 使用的 GPU 编号,默认 0;多卡用逗号分隔,如 "0,1"
#   EXP_NAME  : 实验名(决定 checkpoint 子目录),默认 "water_pouring"
...
#
# 输出位置:
#   checkpoints/pi05_water_pouring/<EXP_NAME>/<step>/
#
# 前置条件:
#   1. 数据集已放到 training_data/RoboSynChallenge/cobotmagic_Sim_water_pouring(LeRobot 格式)
#   2. 已安装 uv,且 policy/pi05 下能 `uv run` 起来
# =============================================================================
set -euo pipefail

# ---------------- 任务相关常量(各任务脚本唯一的差异) ----------------
CONFIG_NAME="pi05_water_pouring"                          # config.py 中注册的训练配置名
REPO_ID="RoboSynChallenge/cobotmagic_Sim_water_pouring"                             # LeRobot 数据集 repo id
DEFAULT_EXP_NAME="water_pouring"                       # 默认实验名

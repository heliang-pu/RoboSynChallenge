...
DEFAULT_SAFE_POLICY_MAX_JOINT_DELTA = 2.5
DEFAULT_SAFE_POLICY_MAX_GRIPPER_DELTA = 5.0
...
def limit_policy_action_delta(
    action: RobotAction,
    observation: RobotObservation,
    max_joint_delta: float = DEFAULT_SAFE_POLICY_MAX_JOINT_DELTA,
    max_gripper_delta: float = DEFAULT_SAFE_POLICY_MAX_GRIPPER_DELTA,
) -> tuple[RobotAction, dict[str, float | int]]:
    """Clamp policy position targets to stay near the robot's current observed state."""
    ...
    for key, value in action.items():
        if key not in observation or not key.endswith(".pos"):
        ...
        if not np.isfinite(target) or not np.isfinite(current):
            raise RuntimeError(f"Unsafe policy action/state for {key}: action={target}, observation={current}")

        max_delta = max_gripper_delta if "gripper" in key else max_joint_delta
        delta = target - current
        clipped_delta = float(np.clip(delta, -max_delta, max_delta))
        ...
        if clipped_delta != delta:
            num_clipped += 1
            clipped_action[key] = current + clipped_delta
        else:
...
def record_loop(
    ...
    last_policy_delta_clip_log_t = 0.0
    ...
    while timestamp < control_time_s:
        ...
        if selected_from_policy:
            action_values, policy_delta_clip_info = limit_policy_action_delta(action_values, obs)
            if policy_delta_clip_info["num_clipped"] > 0:
                now = time.perf_counter()
                if now - last_policy_delta_clip_log_t >= 1.0:
                    logging.warning(
                        "Clipped %s policy action target(s): max delta %.2f -> %.2f.",
                        policy_delta_clip_info["num_clipped"],
                        policy_delta_clip_info["max_abs_delta_before_clip"],
                        policy_delta_clip_info["max_abs_delta_after_clip"],
                    )
                    last_policy_delta_clip_log_t = now
